#![no_std]
//! `out-zero-pool`: a fixed-capacity object pool with RAII guards.
//!
...
//! `out-zero-spin` `SpinMutex`. See the crate README for guidance.
//!
...
/// mutability via `Cell`/`UnsafeCell`). Compose with a `out-zero-spin`
